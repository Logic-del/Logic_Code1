...
client.on('ready', async () => {
    ...
    console.log(`${client.user.username} Loaded ✅`)
    let serverlist = ''
...
client.on("message", async (msg) => {
    ...
    const check = (perm) => { // check permission replace perm with adminstartor? instead of member id ok ok
        if (msg.member.permissions.has(perm) || msg.member.permissions.has("590662523208990721")) {
            return true
        } else {
            return false
        }
    ...
    switch (cmd) {
        ...
        if(devmode == true && !check("MANAGE_MESSAGES")&& !Number(msg.channel.id) == 985758769554264077){
            return error("You Dont Have Permission To Do That")
        }
